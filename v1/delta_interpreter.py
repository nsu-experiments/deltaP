    ...
    def p_expr(self, p):
        ...
        if len(p) == 2:
            ...
            if tok_type == 'NUMBER':
                ...
                p[0] = ConstExpr(p[1])   # p[1] уже без кавычек
            elif tok_type == 'TRUE':
                p[0] = ConstExpr(1.0)
            elif tok_type == 'FALSE':
                p[0] = ConstExpr(0.0)
                ...
                # fallback – например, для UNDEF и т.п., но их в одиночном expr быть не должно
...
class Interpreter:
    ...
    def eval_expr(self, expr: Expr, local: Dict) -> Any:
        ...
        if isinstance(expr, CompareExpr):
            ...
            if expr.op == 'in':
                ...
                raise RuntimeError("'in' right side must be list/range")
        ...
        if isinstance(expr, QuantExpr):
            ...
            if not isinstance(domain, list):
                raise RuntimeError("Quantifier domain must be a list")
            vals = []
            for val in domain:
        ...
        if isinstance(expr, PredicateExpr):
            ...
            if name in self.sp_defs:
                ...
                if len(params) != len(args):
                    raise RuntimeError(f"Arity mismatch for static {name}")
                new_local = local.copy()
            ...
            if name in self.dp_defs:
                ...
                if len(params) != len(args):
                    raise RuntimeError(f"Arity mismatch for dynamic {name}")
                new_local = local.copy()
                for p, a in zip(params, args):
                    new_local[p] = a
    ...
    def execute_statement(self, stmt: Statement):
        if isinstance(stmt, AssignStmt):
            ...
            self.mode = stmt.mode
        elif isinstance(stmt, (AndSemDecl, OrSemDecl, ImplySemDecl)):
            self.mode = lambda x, y: stmt.expr
        elif isinstance(stmt, NotSemDecl):
            self.mode = lambda x: stmt.expr    
            ...
            if not isinstance(coll, list):
                raise RuntimeError("For loop collection must be a list")
            for val in coll:
            ...
            if pred not in self.dp_defs:
                raise RuntimeError(f"Dynamic predicate {pred} not declared")
            params, _ = self.dp_defs[pred]
            args = [self.eval_expr(a, {}) for a in stmt.args]
            if len(args) != len(params):
                raise RuntimeError(f"Arity mismatch for {pred}")
            for a in args:
                if not isinstance(a, int):
                    raise RuntimeError("Dynamic predicate args must be integers")
            ...
            if stmt.name in self.dp_defs:
                ...
                if old_arity != new_arity:
                    raise RuntimeError(f"Cannot change arity of dynamic predicate {stmt.name} from {old_arity} to {new_arity}")
